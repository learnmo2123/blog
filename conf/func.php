...
/**
 * 获取分页HTML部分的函数
 * @param    $totalPage   int    分页总页数
 * @param    $url   int    分页需要跳转的链接地址，例：$url='http://www.blog32.com/index.php?p=admin&m=article&a=showList';
 * @param    $nowPage   int    当前页页码
 *  使用说明：分页链接地址中有一个GET参数名为page，表示跳转的那个页的页码
 */
function pageHtml($totalPage, $url, $nowPage=1,$url2,$url3){
    ...
    if( $nowPage!=1 ){
        ...
        $pageHtml .= "<a href=".$url2.">上一页</a>";
    }

    for($i=1; $i<=$totalPage; $i++ ){

        if( $nowPage==$i ){
            $pageHtml .= "<a href='{$url}&page={$i}' style='color:red;'>{$i}</a>&nbsp;";
        }else{
